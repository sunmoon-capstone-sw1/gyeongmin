...
class Triton:
    ...
    def parse_model(self):
        # print(len(self.model_metadata.outputs))
        if len(self.model_metadata.inputs) != 1:
            raise Exception("expecting 1 input, got {}".format(
                len(self.model_metadata.inputs)))
        if len(self.model_metadata.outputs) != 2:
    ...
    def postprocess(self, results, output_name, batch_size, batching):
        ...
        print(output_array)
        print(int.from_bytes(output_array, "big"))
        print(int.from_bytes(output_array, "little"))
    ...
    def make_model_metadate(self):
        try:
            self.model_metadata = self.triton_client.get_model_metadata(
                model_name=self.MODEL_NAME, model_version=self.MODEL_VERSION, headers=self.HEADERS)
        ...
        while not self.last_request:
            ...
            try:
                for inputs, outputs, model_name, model_version in self.requestGenerator(
                    ...
                    print("Length of response:", len(self.responses))
